...
gcloud beta container clusters create $CLUSTER_NAME \
  ...
  --cluster-ipv4-cidr=10.1.0.0/21 --services-ipv4-cidr=10.2.0.0/19 --default-max-pods-per-node=110 --enable-ip-alias \
  --zone=$ZONE \
...
gcloud compute firewall-rules create $FIREWALL_NAME-ingress \
