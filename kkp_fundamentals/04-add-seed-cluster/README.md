...
```text
{"level":"info","time":"2021-07-06T09:23:08.710Z","caller":"cli/hello.go:36","msg":"Starting Kubermatic Seed Controller-Manager (Community Edition)...","worker-name":"","version":"v2.20.2"}
...
```
>NOTE: For AWS block storage : [Kubermatic Docs - Install Kubermatic - Configure Cluster Backups](https://docs.kubermatic.com/kubermatic/master/installation/install_kkp_ce/add_seed_cluster_ce//#:~:text=configure%20cluster%20backups)
...
```bash
cd $TRAINING_DIR
helm upgrade --install --create-namespace --wait --values ./src/kkp-setup/values.yaml --namespace minio minio ./src/kkp-setup/releases/v2.20.2/charts/minio
helm upgrade --install --wait --values ./src/kkp-setup/values.yaml --namespace kube-system s3-exporter ./src/kkp-setup/releases/v2.20.2/charts/s3-exporter/
```
